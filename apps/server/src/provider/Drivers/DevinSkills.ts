 ...
 *
 * Devin discovers Agent Skills recursively from user and project roots but
...
interface DevinSkillScanBudget {
...
function parseSkillFrontmatter(contents: string): DevinSkillFrontmatter | undefined {
...
const discoverSkillsInRoot = Effect.fn("discoverDevinSkillsInRoot")(function* (input: {
  readonly directory: string;
  readonly scope: "user" | "project";
  readonly budget: DevinSkillScanBudget;
  ...
  const visit = Effect.fn("visitDevinSkillDirectory")(function* (
    directory: string,
    depth: number,
...
const inspectDevinSkills = Effect.fn("inspectDevinSkills")(function* (
  ...
  const userRootsBelow = (base: string) => [
    { directory: path.join(base, ".devin", "skills"), scope: "user" as const },
    { directory: path.join(base, ".cognition", "skills"), scope: "user" as const },
    { directory: path.join(base, ".agents", "skills"), scope: "user" as const },
  ];
  const projectRootsBelow = (base: string) => [
    { directory: path.join(base, ".devin", "skills"), scope: "project" as const },
    { directory: path.join(base, ".cognition", "skills"), scope: "project" as const },
    { directory: path.join(base, ".agents", "skills"), scope: "project" as const },
  ];
  const roots = [...(cwd ? projectRootsBelow(cwd) : []), ...userRootsBelow(userHome)];

  const skillsByName = new Map<string, ServerProviderSkill>();
  const budget: DevinSkillScanBudget = {
