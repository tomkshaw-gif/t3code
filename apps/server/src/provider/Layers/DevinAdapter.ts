...
import { resolveDevinAcpBaseModelId } from "./DevinProvider.ts";
import { type EventNdjsonLogger, makeEventNdjsonLogger } from "./EventNdjsonLogger.ts";
import {
  discoverDevinSkills,
  hasDevinSkillMention,
  rewriteDevinSkillMentions,
} from "../Drivers/DevinSkills.ts";
...
interface DevinSessionContext {
  ...
  devinSkillNames: ReadonlySet<string> | undefined;
...
function resolveRequestedModeId(input: {
  ...
  // Devin enforces approvals through `session/request_permission`, which T3
  // answers per turn. Sessions stay on the Code (`accept-edits`) mode so the
  // agent can propose edits; `ask` would forbid code changes entirely.
  // Full access maps to Bypass Permissions.
  if (input.runtimeMode === "full-access") {
    return (
      findModeByAliases(modeState.availableModes, ["bypass"])?.id ??
    ...
    const sendTurn: DevinAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          if (rawPrompt) {
            let devinSkillNames = ctx.devinSkillNames;
            if (hasDevinSkillMention(rawPrompt) && devinSkillNames === undefined) {
              const skills = yield* discoverDevinSkills(ctx.session.cwd, options?.environment).pipe(
                Effect.provideService(FileSystem.FileSystem, fileSystem),
                Effect.provideService(Path.Path, path),
              );
              devinSkillNames = new Set(
                skills
                  .filter((skill) => skill.enabled && skill.userInvocable !== false)
                  .map((skill) => skill.name),
              );
              ctx.devinSkillNames = devinSkillNames;
            }
            const prompt = devinSkillNames
              ? rewriteDevinSkillMentions(rawPrompt, devinSkillNames)
              : rawPrompt;
